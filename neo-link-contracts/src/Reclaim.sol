...
contract Reclaim is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    function getMerkelizedUserParams(string memory provider, string memory params) external view returns (bool) {
    ...
    function merkelizeUser(Proof memory proof, uint256 _identityCommitment) external nonReentrant {
        uint256 groupId = calculateGroupIdFromProvider(proof.claimInfo.provider);
        bytes32 userParamsHash = calculateUserParamsHash(proof.claimInfo.provider, proof.claimInfo.parameters);
    ...
    function verifyMerkelIdentity(
        ...
        require(dappIdToExternalNullifier[dappId] == _externalNullifier, "Dapp Not Created");
        uint256 groupId = calculateGroupIdFromProvider(provider);
        try SemaphoreInterface(semaphoreAddress).verifyProof(
            groupId, _merkleTreeRoot, _signal, _nullifierHash, _externalNullifier, _proof
        ) {
            return true;
        } catch {
            return false;
     ...
     */
    function calculateGroupIdFromProvider(string memory provider) internal pure returns (uint256) {
        bytes memory providerBytes = bytes(provider);
        bytes memory hashedProvider = abi.encodePacked(keccak256(providerBytes));
        uint256 groupId = BytesUtils.bytesToUInt(hashedProvider, hashedProvider.length - 4);
        return groupId;
    }

    function calculateUserParamsHash(string memory provider, string memory params) internal pure returns (bytes32) {
